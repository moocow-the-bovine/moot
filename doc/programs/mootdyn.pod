...
'mootdyn' is a dynamic Hidden Markov Model (HMM) tagger / disambiguator
program based on the 'libmoot' library.

Calling conventions largely the same as for the L<moot> program.
...
#--------------------------------------------------------------
# Option-Group Dynamic HMM Options
#--------------------------------------------------------------
=pod

=head2 Dynamic HMM Options

=over 4

=item C<--dyn-class=CLASS> , C<-DCLASS>
...
See C<--dyn-invert> for details on how C<Z(w,t)> is estimated.
...
The Maxwell-Boltzmann estimator constants BASE and BETA are given
by the C<--dyn-base> and C<--dyn-beta> args.

=item MIParser

Uses n-gram model data to break input sentences into binary-branching
trees.  If the C<--dyn-text-tags> flag is given, n-gram model is assumed
...
=item C<--dyn-invert=BOOL> , C<-iBOOL>
...
=item C<--dyn-base=FLOAT> , C<-bFLOAT>

Base for Maxwell-Boltzmann estimator (default=2)

Default: '2.0'

See the 'Boltzmann' estimator under C<--dyn-class> for details.




=item C<--dyn-beta=FLOAT> , C<-BFLOAT>
...
=item C<--dyn-freq-eps=FLOAT> , C<-EFLOAT>
...
=item C<--dyn-text-tags> , C<-x>

Use token text field as n-gram source for MIParser

Default: '0'

See the 'MIParser' class under C<--dyn-class> for details.
