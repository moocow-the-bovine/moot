...
waste is a low-level rule-based codec for the moot/WASTE HMM
tokenizer system.
It functions either as an encoder, taking as input one or more 'raw' files, and produces
...
in a pipeline, e.g.
  ...
  waste --scan --lex --aabbr.lex -jconj.lex -wstop.lex INFILE.txt -Owd,loc \\
  | moot -Iwd,loc --stream --model=MODEL --beam-width=100 - \\
  | waste --decode -Iwd,loc -Owd,loc -o OUTFILE.wd
