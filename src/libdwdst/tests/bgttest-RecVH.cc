...
public:
  ...
  typedef hash_map<TagID,ProbT> BgSubtable;
...
public:
  ...
  inline const ProbT lookup(const TagID ptagid, const TagID tagid) const
  {
    ...
    BgSubtable::const_iterator bgsi = bgs.find(tagid);
    if (bgsi == bgs.end()) return 0;
    return bgsi->second;
  };
  ...
  inline void add(const TagID ptagid, const TagID tagid, const ProbT count)
  {
    ...
    BgSubtable::iterator bgsi = bgs.find(tagid);
    if (bgsi == bgs.end()) {
      bgs[tagid] = 0;
    } else {
      bgsi->second += count;
