...
four different types of cooked text file: in order of ascending
...
filenames carry the extension ".t".  The syntax is:
 ...
 RARE_FILE  ::= {RARE_LINE}*
 RARE_LINE  ::= {COMMENT} | {EOS} | {RARE_TOKEN}
 COMMENT    ::= {SPACE}* "%%" ([^{NEWLINE}])*  {NEWLINE}
 EOS        ::= ( {SPACE}* {NEWLINE} )+
 RARE_TOKEN ::= {TOKEN_TEXT} {NEWLINE}
...
By convention, "medium rare" filenames carry the extension ".mt"
 ...
 MED_RARE_LINE  ::= {COMMENT} | {EOS} | {MED_RARE_TOKEN}
...
(tagger output) or ".ttt" (gold standard).
 ...
 MEDIUM_LINE    ::= {COMMENT} | {EOS} | {MEDIUM_TOKEN}
...
(tagger output) or ".dttt" (gold standard).
 ...
 WELL_DONE_LINE  ::= {COMMENT} | {EOS} | {WELL_DONE_TOKEN}
