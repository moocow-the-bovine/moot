...
public:
  ...
  class ViterbiNode : public dwdstHMM::ViterbiNode
  {
  public:
...
public:
  ...
  /** Tag tokens from a C-stream using dwdstTaggerLexer */
  ...
  inline void tag_print_best_path(FILE *out=stdout)
  {
	...
	carp("%s: Error: no best tag for token '%s'!\n",
	     "dwdstCHMM::tag_print_best_path()", toki->c_str());
   ...
   */
  inline TagMorphMap *token2morphmap(const dwdstTokString &token, TokID tokid=0)
  {
   ...
   */
  inline TagMorphMap *get_token_morphmap(const dwdstTokString &token, TagMorphMap *tmm=NULL)
  {
