 ...
 *     - raw text (no markup!)
 *   + process with Coetmeur's flex++ to produce 'mootTaggerLexer.cc'
 *----------------------------------------------------------------------*/

/* --- Lexer name --- */
%name mootTaggerLexer

%define CLASS mootTaggerLexer
%define MEMBERS \
  public: \
    /* -- public typedefs */\
...
/*----------------------------------------------------------------------
 * mootTaggerLexer::step_streams(FILE *in, FILE *out)
 *   + hack for non-global yywrap()
 *----------------------------------------------------------------------*/
void mootTaggerLexer::step_streams(FILE *in, FILE *out)
{
  yyin = in;
  yyout = out;
  lasttok = DTEOF;
