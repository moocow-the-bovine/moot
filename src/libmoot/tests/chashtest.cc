 ...
 *----------------------------------------------------------------------*/
#define BENCH_CLASS mootCHMM
class BENCH_CLASS : public mootHMM {
public:
  /** Typedef for a class-ID */
  typedef set<FSM::FSMSymbolVector> LexClass;

  /** Typedef for a class probability lookup table (see also mootHMM::LexProbSubTable) */
...
public:
  ...
  bool compile(const mootLexfreqs &lexfreqs,
	       ...
	       const mootLexfreqs::LexfreqCount unknownLexThreshhold=1);
}

bool BENCH_CLASS::compile(const mootLexfreqs &lexfreqs,
			  const mootNgrams   &ngrams,
			  const mootTagString &start_tag_str,
			  const mootLexfreqs::LexfreqCount unknownLexThreshhold)
{
  //-- superclass compilation
  if (!mootHMM::compile(lexfreqs,ngrams,start_tag_str,unknownLexThreshhold))
    return false;
  ...
  mootLexfreqs::LexfreqStringTable::const_iterator lfti;
  LexClass lclass;
  for (lfti = lexfreqs.lftable.begin(); lfti != lexfreqs.lftable.end(); lfti++)
    {
      const mootTokString &tokstr = tgr.tag_token(lfti->first.first);
      const mootTokString &tagstr = tgr.tag_token(lfti->first.second);
      ...
      const mootTagger::MorphAnalysisSet &ma = tgr.tag_token(tokstr);
      lclass.clear();
      for (mootTagger::MorphAnalysisSet::const_iterator mai = ma.begin();
	   mai != ma.end();
	   mai++)
	{
